...
void main() {
    if (uBlurKerSize == 5)
        oBlurred = blur5(uSrcTex, texCoord, uDirection);
    else if (uBlurKerSize == 9)
        oBlurred = blur9(uSrcTex, texCoord, uDirection);
    else
        oBlurred = blur13(uSrcTex, texCoord, uDirection);
}